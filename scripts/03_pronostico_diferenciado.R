# ==============================================================================
# SCRIPT: 03_pronostico_diferenciado.R 
# PROYECTO: Análisis de Logs de Red
# DESCRIPCIÓN: Modelado sobre serie diferenciada.
# ==============================================================================

# --- [1] Preparación y Partición de Datos ---
# Asegúrate de que 'traffic_diff_min' esté cargado del script anterior
n <- length(traffic_diff_min)
n_train <- floor(n * 0.8)
train_ts <- subset(traffic_diff_min, end = n_train)
test_ts  <- subset(traffic_diff_min, start = n_train + 1)
...
tabla_comparativa_diferenciada <- rbind(metrics_hw, metrics_arima, metrics_sarima) %>%
  arrange(RMSE) # El mejor modelo tendrá el RMSE más bajo

# Guardar tabla de resultados
write.csv(tabla_comparativa_diferenciada, file.path(path_output, "12_comparacion_modelos_diferenciado.csv"), row.names = FALSE)

# Identificar el mejor modelo
mejor_modelo_nombre_diferenciado <- tabla_comparativa_diferenciada$Modelo[1]
fc_mejor <- if(mejor_modelo_nombre_diferenciado == "Auto-ARIMA") fc_arima else 
  if(mejor_modelo_nombre_diferenciado == "Holt-Winters") fc_hw else fc_sarima

# --- [6] Validación de Supuestos del Mejor Modelo ---
# Validamos residuales
png(file.path(path_output, "11_diagnostico_mejor_modelo_diferenciado.png"), width = 1000, height = 800, res = 120)
...
p_6 <- autoplot(train_ts) +
  ...
  labs(title = "Validación Final del Modelo de Tráfico",
       subtitle = paste("Ganador:", mejor_modelo_nombre_diferenciado, "| RMSE:", round(tabla_comparativa_diferenciada$RMSE[1], 2)),
...
cat("================================================\n")
print(tabla_comparativa_diferenciada)
cat("\nEl mejor modelo seleccionado es:", mejor_modelo_nombre_diferenciado, "\n")
